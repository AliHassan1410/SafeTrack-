...
export const googleSignIn = async (req, res) => {
  try {
    const { idToken, role, isMock, mockEmail, mockName, mockPicture } = req.body;
    ...
    // 2️⃣ Verify Google ID Token with Google's servers or use Mock
    let googlePayload;
    if (isMock) {
      // ONLY allow mock sign-in if not in production
      if (process.env.NODE_ENV === "production") {
        return res.status(400).json({ message: "Mock authentication is disabled in production" });
      }
      googlePayload = {
        sub: `mock-google-id-${Date.now()}`,
        email: mockEmail || "test@gmail.com",
        name: mockName || "Mock User",
        picture: mockPicture || null,
        email_verified: true,
      };
    } else {
      try {
