...
import 'package:safetrack/services/notification_service.dart';
...
class _ResponderHomeState extends State<ResponderHome> {
  ...
  void _initSocket() {
    ...
    _socket.on('new_notification', (data) {
      ...
      if (data['recipient'] == AuthService().currentUser?.uid) {
        if (mounted) {
          setState(() {
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      String address = "${_currentPosition!.latitude.toStringAsFixed(4)}, ${_currentPosition!.longitude.toStringAsFixed(4)}";
      try {
      ...
      if (mounted) {
        setState(() {
          ...
          _notificationCount = notifications.where((n) => n['isUnread'] == true).length;
                          ...
                          onPressed: () async {
                            try {
                              ...
                              if (mounted) {
                               ...
                               ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Failed to mark read')));
                            }
                          },
              ...
              Expanded(
                child: _notifications.isEmpty 
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return _professionalNotificationItem(
                      icon: icon,
                      iconColor: iconColor,
              ...
              children: [
                ...
                Expanded(
                  child: OutlinedButton.icon(
                    onPressed: () {
                      if (id.isNotEmpty) {
                        final incidentData = _incidents.firstWhere((i) => i['_id'] == id, orElse: () => null);
                        if (incidentData != null) {
                           final incTitle = incidentData['title'] ?? title;
                           final description = incidentData['description'] ?? 'No description available.';
                           String timeText = "Unknown Time";
                           if (incidentData['createdAt'] != null) {
                              try {
                                final dt = DateTime.parse(incidentData['createdAt']).toLocal();
                                final ampm = dt.hour >= 12 ? 'PM' : 'AM';
                                int hourStr = dt.hour % 12;
                                if (hourStr == 0) hourStr = 12;
                           ...
                           if (incidentData['reporter'] != null && incidentData['reporter'] is Map) {
                              repName = incidentData['reporter']['name'] ?? "Unknown Reporter";
                              repPhone = incidentData['reporter']['phone'] ?? "No Phone";
                           }
                           final incCode = "INC-${id.length >= 4 ? id.substring(id.length - 4).toUpperCase() : id}";
                           _showIncidentDetails(incCode, incTitle, description, location, timeText, repName, repPhone);
                        }
                      }
                    },
                    style: OutlinedButton.styleFrom(
