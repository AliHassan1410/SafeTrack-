...
class _ResponderProfilePageState extends State<ResponderProfilePage> {
  ...
  String _badgeNumber = "Not Provided";
  ...
  Future<void> _loadProfileData() async {
    ...
    if (user != null) {
      ...
      if (user.responderType == "medical") {
        _department = "Punjab Emergency Service (Rescue 1122)";
      } else if (user.responderType == "crime") {
        _department = "Police Department";
      } else {
        _department = "Emergency Responder";
      }
    }

    try {
      final incidents = await IncidentService.getAssignedIncidents();
      int completed = incidents.where((i) => i['status'] == 'resolved' || i['status'] == 'accepted').length;
      _totalIncidents = completed;
    } catch (e) {
      ...
      child: Row(
        children: [
          _StatBox(
            ...
            value: _isLoading ? "..." : "$_totalIncidents",
            color: AppColors.primary,
