...
class _ReporterHomeState extends State<ReporterHome>
  ...
  // _showQuickTextDialog removed

  String _determineCategory(String text) {
    text = text.toLowerCase();
    
    List<String> medicalKeywords = [
      "accident", "injury", "hurt", "bleeding", "blood",
      "unconscious", "heart attack", "chest pain",
      "ambulance", "hospital", "doctor",
      "breathing", "sick", "pain", "collapse",
      "medical", "patient"
    ];

    List<String> crimeKeywords = [
      "theft", "steal", "stolen", "chori", "robbery",
      "burglary", "snatching", "mugging", "attack",
      "police", "crime", "criminal", "suspect",
      "gun", "knife", "weapon", "fight", "assault",
      "kidnap", "abduction", "threat", "violence",
      "shooting", "gang", "danger"
    ];
    ...
    for (String keyword in crimeKeywords) {
      if (text.contains(keyword)) {
        return "Crime";
      }
    }
  ...
  void _showQuickVoiceDialog() {
    ...
    showDialog(
      ...
      if (result != null && result is String && result.isNotEmpty) {
        String category = _determineCategory(result);
        
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => ReportIncidentScreen(
            initialTabIndex: 0,
            initialDescription: result,
...
class _QuickVoiceDialogState extends State<QuickVoiceDialog> {
  ...
  Future<void> _initSpeechAndListen() async {
    try {
      ...
      if (_hasSpeech && mounted) {
        setState(() {});
        _listen(); // Auto-start recording immediately
      }
    } catch (e) {
