...
class IncidentService {
  ...
  /* ---------------------------------------
     ✅ COMPLETE INCIDENT (RESPONDER)
  ----------------------------------------*/
  static Future<void> completeIncident(String incidentId) async {
    try {
      final token = await _getToken();

      final response = await http.put(
        Uri.parse("$baseUrl/incidents/$incidentId/complete"),
        headers: {
          "Authorization": "Bearer $token",
        },
      );
