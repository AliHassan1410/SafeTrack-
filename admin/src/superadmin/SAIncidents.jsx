// Super Admin — Incidents page matching regular Incidents.jsx layout exactly with all charts and layouts
import { useEffect, useState } from 'react'
import {
  saGetIncidents,
  saGetResponders,
  saUpdateIncidentStatus,
  saAssignIncidentResponder,
...
export default function SAIncidents() {
  ...
  const load = () => {
    setLoading(true)
    Promise.all([saGetIncidents(), saGetResponders()])
      .then(([incRes, resRes]) => {
  ...
  const handleStatusChange = async (id, newStatus) => {
    ...
    try {
      await saUpdateIncidentStatus(id, newStatus)
  ...
  const handleAssignResponder = async (id, responderId) => {
    ...
    try {
      const res = await saAssignIncidentResponder(id, responderId)
  ...
  const downloadCSV = () => {
    ...
    link.href = url
    link.setAttribute('download', `safetrack_all_incidents_${new Date().toISOString().split('T')[0]}.csv`)
    ...
    >
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Super Admin global monitoring and response dispatch</p>
        </div>
