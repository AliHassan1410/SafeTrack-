// Super Admin Profile — mirrors AdminProfile.jsx layout exactly, uses SA context
import { useState, useEffect } from 'react'
import { useSuperAdminAuth } from '../context/SuperAdminAuthContext'
import { superApi } from '../context/SuperAdminAuthContext'
import { saUpdateProfile } from '../services/superAdminApi'
import toast from 'react-hot-toast'
import { User, Phone, Mail, Camera, Shield, Calendar, Lock } from 'lucide-react'
...
export default function SAAdminProfile() {
  const { admin, updateAdmin } = useSuperAdminAuth()
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        const res = await superApi.get('/api/auth/profile')
        const user = res.data
        setName(user.name || '')
        setPhone(user.phone || '')
        ...
        console.error('Failed to load SA profile:', err)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = { name, phone, profilePic }
      if (newPassword) {
        if (!currentPassword) {
          toast.error('Enter current password to change it', { id: toastId })
          setSaving(false)
          return
        }
      ...
      const res = await saUpdateProfile(payload)
      updateAdmin({ ...admin, ...res.data.user })
          ...
          <div className="profile-sidebar-card">
            <div className="avatar-upload-container">
              <div className="avatar-preview-wrap">
                {profilePic ? (
                  ...
                  <div className="avatar-placeholder" style={{ background: 'linear-gradient(135deg, #8b5cf6, #6d28d9)' }}>
                    {name.charAt(0).toUpperCase() || 'S'}
                  </div>
                )}
                <label htmlFor="sa-avatar-file-input" className="avatar-camera-btn" title="Upload custom photo">
                  <Camera size={16} />
                  <input
                    id="sa-avatar-file-input"
                    type="file"
                    accept="image/*"
              ...
              <h3>{name || 'Super Administrator'}</h3>
              <span className="profile-badge" style={{ background: 'rgba(139,92,246,0.15)', color: '#a78bfa', border: '1px solid rgba(139,92,246,0.3)' }}>
                👑 Super Admin
              </span>
            </div>
            ...
            <div className="profile-meta-info">
              <div className="meta-row">
                ...
                <span>Role: <strong style={{ color: '#a78bfa' }}>Super Administrator</strong></span>
              </div>
              <div className="meta-row">
                <Calendar size={14} className="meta-icon" />
                <span>Status: <strong style={{ color: 'var(--accent-green)' }}>Active ✓</strong></span>
              </div>
              <div className="meta-row">
          ...
          <div className="profile-edit-card">
            ...
            <div className="form-fields-container">
              ...
              <div style={{ borderTop: '1px solid var(--border)', paddingTop: 20, marginTop: 4 }}>
                <p className="field-label" style={{ marginBottom: 16, fontSize: 14, color: 'var(--text-secondary)' }}>
                  <Lock size={14} style={{ display: 'inline', marginRight: 6 }} />
                  Change Password <span style={{ fontWeight: 400, color: 'var(--text-muted)' }}>(optional)</span>
                </p>
