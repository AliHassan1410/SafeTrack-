...
import { saGetResponders, saSuspendUser, saUnsuspendUser, saCreateResponder } from '../services/superAdminApi'
import { motion } from 'framer-motion'
import { Users, Plus, ShieldAlert, Phone } from 'lucide-react'
import toast from 'react-hot-toast'
import '../pages/Responders.css'
...
export default function SAResponders() {
  ...
  const handleRegister = async (e) => {
    ...
    setRegistering(true)
    try {
      const res = await saCreateResponder({ name, email, password, phone, responderType })
    ...
    <motion.div initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.3 }}>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Manage emergency responders — register and monitor personnel city-wide</p>
        </div>
      ...
      {error && <div style={{ background:'var(--accent-red-bg)', border:'1.5px solid rgba(239,68,68,0.3)', color:'var(--accent-red)', padding:'12px 18px', borderRadius:'var(--radius-md)', marginBottom:20, fontSize:13 }}>{error}</div>}
